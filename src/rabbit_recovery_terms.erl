...
start(VHost) ->
    rabbit_log:info("Getting vhost sup for vhost ~p~n", [VHost]),
    {ok, VHostSup} = rabbit_vhost_sup_sup:vhost_sup(VHost),
    rabbit_log:info("Starting recovery terms at vhost sup ~p~n", [VHostSup]),
...
start_link(VHost) ->
    rabbit_log:info("Starting recovery terms child"),
...
init([VHost]) ->
    ...
    rabbit_log:info("Open recovery terms table"),
    open_table(VHost),
    rabbit_log:info("recovery terms table opened"),
    {ok, VHost}.
